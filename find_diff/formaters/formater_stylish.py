...
def get_diff_stylish(diff_dict: dict) -> str:
    ...
    def build_diff_string(diff_dict: dict, depth: int = 1, indent: int = 4) -> str:
        diff_string = ""

        for key, values in diff_dict.items():
            nested_indent = " " * (indent * depth)

            if isinstance(values, dict) and not {DICT1, DICT2, BOTH} & set(values):
            ...
            for prefix, dict_key in [("-", DICT1), ("+", DICT2)]:
                if dict_key in values and isinstance(values[dict_key], dict):
                    nested_diff = dic_to_string(values[dict_key], depth, indent)
                    diff_string += f"{nested_indent[2:]}{prefix} {key}: {nested_diff}\n"
                elif dict_key in values:
                    diff_string += f"{nested_indent[2:]}{prefix} {key}: {values[dict_key]}\n"
